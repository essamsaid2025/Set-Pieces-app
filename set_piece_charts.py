...
class SimplePitch:
    ...
    def draw(self, ax):
        ...
        cx = self.pitch_length / 2
        cy = self.pitch_width / 2

        ax.add_patch(
            plt.Circle((cx, cy), 9.15, fill=False, color=self.line_color, lw=self.linewidth, zorder=self.line_zorder)
        )

        ax.add_patch(
            Rectangle((0, (self.pitch_width - 40.32) / 2), 16.5, 40.32, fill=False,
                      edgecolor=self.line_color, linewidth=self.linewidth, zorder=self.line_zorder)
        )
        ax.add_patch(
            Rectangle((0, (self.pitch_width - 18.32) / 2), 5.5, 18.32, fill=False,
                      edgecolor=self.line_color, linewidth=self.linewidth, zorder=self.line_zorder)
        )

        ax.add_patch(
            Rectangle((self.pitch_length - 16.5, (self.pitch_width - 40.32) / 2), 16.5, 40.32, fill=False,
                      edgecolor=self.line_color, linewidth=self.linewidth, zorder=self.line_zorder)
        )
        ax.add_patch(
            Rectangle((self.pitch_length - 5.5, (self.pitch_width - 18.32) / 2), 5.5, 18.32, fill=False,
                      edgecolor=self.line_color, linewidth=self.linewidth, zorder=self.line_zorder)
        )

        ax.scatter([11, self.pitch_length - 11], [cy, cy], c=self.line_color, s=10, zorder=self.line_zorder)

        ax.add_patch(
            Arc((11, cy), 18.3, 18.3, angle=0, theta1=310, theta2=50,
                color=self.line_color, lw=self.linewidth, zorder=self.line_zorder)
        )
        ax.add_patch(
            Arc((self.pitch_length - 11, cy), 18.3, 18.3, angle=0, theta1=130, theta2=230,
                color=self.line_color, lw=self.linewidth, zorder=self.line_zorder)
        )
        ...
        goal_width = 7.32
        ax.add_patch(
            Rectangle((-goal_depth, cy - goal_width / 2), goal_depth, goal_width, fill=False,
                      edgecolor=self.line_color, linewidth=self.linewidth, zorder=self.line_zorder)
        )
        ax.add_patch(
            Rectangle((self.pitch_length, cy - goal_width / 2), goal_depth, goal_width, fill=False,
                      edgecolor=self.line_color, linewidth=self.linewidth, zorder=self.line_zorder)
        )
...
def _corner_anchor(x: float, y: float) -> Tuple[float, float, str]:
    """
    نخلي البداية جوه الملعب بشوية بدل الحرف بالظبط
    """
...
def _draw_box_zone_overlay(ax, style: dict, alpha: float = 0.18):
    """
    Zone overlay جوه البوكس على الجون اليمين
    """
    # near post / central / far post
    x0 = 84
    x1 = 100

    zones = [
        ("Near Post", x0, 0, x1 - x0, 18, style["accent"]),
        ("Central", x0, 18, x1 - x0, 28, style["warning"]),
        ("Far Post", x0, 46, x1 - x0, 18, style["success"]),
    ]
...
def _plot_delivery_trajectories_core(
    ...
    title: str = "Delivery Trajectories",
    ...
    if zone_overlay:
        _draw_box_zone_overlay(ax, style)
...
def chart_delivery_start_map(df: pd.DataFrame, theme_name: str, flip_y: bool = False, style_overrides: Optional[dict] = None):
    ...
    pitch.scatter(
        dd["x_start_plot"], dd["y_start_plot"],
        ax=ax,
        s=style["marker_size"],
        color=style["accent"],
...
def chart_delivery_heatmap(df: pd.DataFrame, theme_name: str, flip_y: bool = False, style_overrides: Optional[dict] = None):
    ...
    pitch.draw(ax=ax)
    style_pitch_axes(ax, style)
    _draw_box_zone_overlay(ax, style, alpha=0.12)
...
def chart_delivery_trajectories(df: pd.DataFrame, theme_name: str, flip_y: bool = False, style_overrides: Optional[dict] = None):
    return _plot_delivery_trajectories_core(
        df=df,
        theme_name=theme_name,
        flip_y=flip_y,
        style_overrides=style_overrides,
        title="Delivery Trajectories",
        corner_filter=None,
        zone_overlay=True,
    )


def chart_delivery_trajectories_left(df: pd.DataFrame, theme_name: str, flip_y: bool = False, style_overrides: Optional[dict] = None):
    return _plot_delivery_trajectories_core(
        df=df,
        theme_name=theme_name,
        flip_y=flip_y,
        style_overrides=style_overrides,
        title="Delivery Trajectories - Left Corners",
        corner_filter="left",
        zone_overlay=True,
    )


def chart_delivery_trajectories_right(df: pd.DataFrame, theme_name: str, flip_y: bool = False, style_overrides: Optional[dict] = None):
    return _plot_delivery_trajectories_core(
        df=df,
        theme_name=theme_name,
        flip_y=flip_y,
        style_overrides=style_overrides,
        title="Delivery Trajectories - Right Corners",
        corner_filter="right",
        zone_overlay=True,
    )
...
def chart_average_delivery_path(df: pd.DataFrame, theme_name: str, flip_y: bool = False, style_overrides: Optional[dict] = None):
    ...
    if "delivery_type" in dd.columns and dd["delivery_type"].notna().any():
        ...
        for dtype, grp in dd.groupby("delivery_type"):
            ...
            pitch.scatter([avg_x2], [avg_y2], ax=ax, s=style["marker_size"] * 1.2,
                          color=color_map.get(str(dtype).lower(), style["accent"]),
...
def chart_heat_plus_trajectories(df: pd.DataFrame, theme_name: str, flip_y: bool = False, style_overrides: Optional[dict] = None):
    ...
    pitch.draw(ax=ax)
    style_pitch_axes(ax, style)
    _draw_box_zone_overlay(ax, style, alpha=0.10)
